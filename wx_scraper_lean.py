...
from datetime.datetime import today as dttoday
import matplotlib.pyplot as plt
#from matplotlib.pyplot.plt import figure, close, title
from bs4 import BeautifulSoup
from glob import glob
from time import sleep
from os import remove, makedirs
from os.path import exists
from pandas import merge, DataFrame, read_csv, to_datetime, date_range
from numpy import nan, mean, linspace, pi, zeros
...
def summarize(df,begin,r_int):
    ...
    df = merge(df_mean,merge(df_min,df_max,left_index=True, right_index=True),left_index=True, right_index=True)
...
def wind_rose(plots_dir,df,p_int,name,latest,city,state,lat,long,elev):
    df.loc[:,'WindDir'] = round((df.WindDir / (360 / p_int)),0) * (360 / p_int)
    ...
    df_deg = DataFrame(data = linspace(0, 360, p_int, endpoint=False), columns = ['WindDir'])
    ...
    theta = linspace(0.0, 2 * pi, p_int, endpoint=False)
...
def wind_date(plots_dir,df,p_int,name,size,latest,city,state,lat,long,elev,label,major,minor):
    ...
    ax1.bar(gustNA.index, zeros(len(gustNA))-height, width, edgecolor='none', color='#3366ff')
    ax1.bar(gust00.index, zeros(len(gust00))-height, width, edgecolor='none', color='#009999')
    ax1.bar(gust01.index, zeros(len(gust01))-height, width, edgecolor='none', color='#00cc00')
    ax1.bar(gust02.index, zeros(len(gust02))-height, width, edgecolor='none', color='#bfff00')
    ax1.bar(gust05.index, zeros(len(gust05))-height, width, edgecolor='none', color='#ffcc00')
    ax1.bar(gust10.index, zeros(len(gust10))-height, width, edgecolor='none', color='#ffff00')
    ax1.bar(windNA.index, zeros(len(windNA))-(height/2), width, edgecolor='none', color='#3366ff')
    ax1.bar(wind00.index, zeros(len(wind00))-(height/2), width, edgecolor='none', color='#009999')
    ax1.bar(wind01.index, zeros(len(wind01))-(height/2), width, edgecolor='none', color='#00cc00')
    ax1.bar(wind02.index, zeros(len(wind02))-(height/2), width, edgecolor='none', color='#bfff00')
    ax1.bar(wind05.index, zeros(len(wind05))-(height/2), width, edgecolor='none', color='#ffcc00')
    ax1.bar(wind10.index, zeros(len(wind10))-(height/2), width, edgecolor='none', color='#ffff00')
    ax2 = ax1.twinx()
    ...
    ylim2max = round(((df.WindspeedGust_max.max()+5) / 5),0) * 5
    ylim2min = -ylim2max/4
    ax2.set_yticks(linspace(0,ylim2max,(ylim2max/4)+1))
    ax2.set_ylabel('Windspeed / Gust (mph)')
    ax2.yaxis.label.set_color('yellowgreen')
    ax1.plot_date(df.index, df.WindDir, 'rx', ms=size)
    ax1.set_yticks(linspace(0,360,(p_int/4)+1))
...
def dTdt_solar_temp(plots_dir,df,name,size,latest,city,state,lat,long,elev):
    plt.close("all")
    fig = plt.figure()
    fig.set_size_inches(7.5, 5)
    ...
    axcb = ax.scatter(df.SolarRadiation, df.dTdt, marker='x', c=df.Temp, vmin=-20, vmax=120, s=size, cmap=plt.cm.jet, alpha=0.8, lw = 1)
...
def dTdt_date(plots_dir,df,name,size,latest,city,state,lat,long,elev,label,major,minor):
    plt.close("all")
    fig = plt.figure()
    fig.set_size_inches(7.5, 5)
    plt.title(sid+' - '+city+', '+state+': '+str(lat)+', '+str(long)+', '+str(elev)+'ft\nTemp + dT/dt - '+name)
    ...
    axcb = ax.scatter(df.index, df.Temp_avg, c=df.dTdt_avg, marker='x', s=size, cmap=plt.cm.jet, vmin=-v, vmax=v, alpha=0.8, lw = 1)
...
def temp_dew_hum(plots_dir,df,name,size,latest,city,state,lat,long,elev):
    plt.close("all")
    fig = plt.figure()
    fig.set_size_inches(7.5, 5)
    ...
    ax.set_axisbelow(True)
    ax.spines['top'].set_visible(True)
    axcb = ax.scatter(df.Dewpoint, df.Temp, c=df.Humidity, marker='x', s=size, cmap=plt.cm.winter, vmin=0, vmax=100, alpha=0.8, lw = 1)
...
def dPdt_wind_temp(plots_dir,df,name,size,latest,city,state,lat,long,elev):
    plt.close("all")
    fig = plt.figure()
    fig.set_size_inches(7.5, 5)
    plt.title(sid+' - '+city+', '+state+': '+str(lat)+', '+str(long)+', '+str(elev)+'ft\nSolar Radiation, dT/dt, Temp - '+name)
    ...
    ax.set_axisbelow(True)
    ax.spines['top'].set_visible(True)
    axcb = ax.scatter(df.Windspeed, df.dPdt, c=df.Temp, marker='x', vmin=-20, vmax=120, s=size, cmap=plt.cm.jet, alpha=0.8, lw = 1)
    ax.set_xlabel('Windspeed (mph)')
    ax.set_ylabel('dP/dt (inHg/hr)')
    ax.grid(b=True, which='both', color='k',linestyle='-')
    cbar = plt.colorbar(axcb)
    cbar.set_label('Temp (degF)')
    fig.text(0.98,0.02,latest,fontsize=7,verticalalignment='bottom',horizontalalignment='right',transform=ax.transAxes)
...
def combo(plots_dir,df,name,lw,latest,city,state,lat,long,elev,label,major,minor):
    plt.close("all")
    fig = plt.figure()
    ...
    ax5.set_yticks(linspace(0,360,(p_int/4)+1))
...
def main(start_date,sid):
    ...
    soup_sd = BeautifulSoup(urlopen(url_sd))
    ...
    start = strptime(start_date, "%Y-%m-%d").date()
    today = dttoday().date()
    now = dttoday()
    ...
    fetch_range = DataFrame(columns=['Date'],data=date_range(start_date, today))
    try:
        data = read_csv(data_dir+sid+'_data.csv',index_col=False)
        data_dates = DataFrame(columns=['Date'],data=to_datetime(data.Time).dt.date.unique())
        recent_range = DataFrame(columns=['Date'],data=date_range(lim_wek, today).date)
    ...
    for date in fetch_dates.Date:
        ...
        soup = BeautifulSoup(urlopen(url)).text
        if exists(file) == True:
    ...
    try:
        for csv1 in glob(data_dir+sid+'*_fetch.csv'):
            data = data.append(read_csv(csv1,index_col=False))
            remove(csv1)
    except:
        data = DataFrame()
        for csv1 in glob(data_dir+sid+'*_fetch.csv'):
    ...
    df['DateUTC'] = to_datetime(df.DateUTC)
    df.index = df.Time
    df = df.sort(['DateUTC'],ascending = True)

    # data treatment
    treat(df,'DewpointF',-50,nan,'Dewpoint')
    treat(df,'HourlyPrecipIn',0,0,'PrecipHourly')
    treat(df,'Humidity',0,nan,'Humidity')
    treat(df,'PressureIn',0,nan,'Pressure')
    treat(df,'SolarRadiationWatts/m^2',-100,nan,'SolarRadiation')
    treat(df,'TemperatureF',-100,nan,'Temp')
    treat(df,'WindDirectionDegrees',-.01,nan,'WindDir')
    treat(df,'WindSpeedGustMPH',-.01,nan,'WindspeedGust')
    treat(df,'WindSpeedMPH',-.01,nan,'Windspeed')
    treat(df,'dailyrainin',-.01,nan,'PrecipDaily')
    df.loc[df['Windspeed'] == 0, 'WindDir'] = nan
    print('data treated')
    ...
    if save_archive == True:
        df.to_csv(archive_dir+sid+'_archive.csv')
        print('data archived')
    else:
    ...
    # create combo plots
    try:
        combo(plots_dir,df_summ_day,'1day',1,latest,city,state,lat,long,elev,label_day,maj_day,min_day)
        combo(plots_dir,df_summ_wek,'1week',1,latest,city,state,lat,long,elev,label_wek,maj_wek,min_wek)
        combo(plots_dir,df_summ_mon,'1month',1,latest,city,state,lat,long,elev,label_mon,maj_mon,min_mon)
        #combo(plots_dir,df_summ_3mo,'3month',1,latest,city,state,lat,long,elev,label_3mo,maj_3mo,min_3mo)
        combo(plots_dir,df_summ_yer,'1year',1,latest,city,state,lat,long,elev,label_all,maj_all,min_all)
        #combo(plots_dir,df_summ_all,'all',1,latest,city,state,lat,long,elev,label_all,maj_all,min_all)
    except:
        pass

    # create windroses
    try:
        wind_rose(plots_dir,df_rawl_day,p_int,'1day',latest,city,state,lat,long,elev)
        wind_rose(plots_dir,df_rawl_wek,p_int,'1week',latest,city,state,lat,long,elev)
        wind_rose(plots_dir,df_rawl_mon,p_int,'1month',latest,city,state,lat,long,elev)
        wind_rose(plots_dir,df_rawl_3mo,p_int,'3month',latest,city,state,lat,long,elev)
        wind_rose(plots_dir,df_rawl_yer,p_int,'1year',latest,city,state,lat,long,elev)
        wind_rose(plots_dir,df_rawl_all,p_int,'all',latest,city,state,lat,long,elev)
        wind_rose(plots_dir,df_rawl_dys,p_int,'days',latest,city,state,lat,long,elev)
        wind_rose(plots_dir,df_rawl_nts,p_int,'nights',latest,city,state,lat,long,elev)
    except:
        pass

    # create wind plots
    try:
        wind_date(plots_dir,df_summ_day,p_int,'1day',4,latest,city,state,lat,long,elev,label_day,maj_day,min_day)
        wind_date(plots_dir,df_summ_wek,p_int,'1week',4,latest,city,state,lat,long,elev,label_wek,maj_wek,min_wek)
        wind_date(plots_dir,df_summ_mon,p_int,'1month',4,latest,city,state,lat,long,elev,label_mon,maj_mon,min_mon)
        wind_date(plots_dir,df_summ_3mo,p_int,'3month',4,latest,city,state,lat,long,elev,label_3mo,maj_3mo,min_3mo)
        wind_date(plots_dir,df_summ_yer,p_int,'1year',4,latest,city,state,lat,long,elev,label_all,maj_all,min_all)
        #wind_date(plots_dir,df_summ_all,p_int,'all',4,latest,city,state,lat,long,elev,label_all,maj_all,min_all)
    except:
        pass

    # create plots
    try:
        dTdt_solar_temp(plots_dir,df_rawl_day,'1day',75,latest,city,state,lat,long,elev)
        dTdt_solar_temp(plots_dir,df_rawl_wek,'1week',75,latest,city,state,lat,long,elev)
        dTdt_solar_temp(plots_dir,df_rawl_mon,'1month',75,latest,city,state,lat,long,elev)
        #dTdt_solar_temp(plots_dir,df_summ_3mo,'3month',75,latest,city,state,lat,long,elev)
        dTdt_solar_temp(plots_dir,df_rawl_yer,'1year',75,latest,city,state,lat,long,elev)
        #dTdt_solar_temp(plots_dir,df_summ_all,'all',75,latest,city,state,lat,long,elev)
    except:
        pass

    # create plots
    try:
        temp_dew_hum(plots_dir,df_rawl_day,'1day',75,latest,city,state,lat,long,elev)
        #temp_dew_hum(plots_dir,df_summ_wek,'1week',75,latest,city,state,lat,long,elev)
        #temp_dew_hum(plots_dir,df_summ_mon,'1month',75,latest,city,state,lat,long,elev)
        #temp_dew_hum(plots_dir,df_summ_3mo,'3month',75,latest,city,state,lat,long,elev)
        temp_dew_hum(plots_dir,df_rawl_yer,'1year',75,latest,city,state,lat,long,elev)
        #temp_dew_hum(plots_dir,df_summ_all,'all',75,latest,city,state,lat,long,elev)
    except:
        pass

    # create plots
    try:
        dTdt_date(plots_dir,df_summ_day,'1day',75,latest,city,state,lat,long,elev,label_day,maj_day,min_day)
        dTdt_date(plots_dir,df_summ_wek,'1week',75,latest,city,state,lat,long,elev,label_wek,maj_wek,min_wek)
        #dTdt_date(plots_dir,df_summ_mon,'1month',75,latest,city,state,lat,long,elev)
        #dTdt_date(plots_dir,df_summ_3mo,'3month',75,latest,city,state,lat,long,elev)
        #dTdt_date(plots_dir,df_summ_yer,'1year',75,latest,city,state,lat,long,elev)
        #dTdt_date(plots_dir,df_summ_all,'all',75,latest,city,state,lat,long,elev)
    except:
        pass
    
    # create plots
    try:
        dPdt_wind_temp(plots_dir,df_rawl_day,'1day',75,latest,city,state,lat,long,elev)
        dPdt_wind_temp(plots_dir,df_rawl_wek,'1week',75,latest,city,state,lat,long,elev)
        dPdt_wind_temp(plots_dir,df_rawl_mon,'1month',75,latest,city,state,lat,long,elev)
        #dPdt_wind_temp(plots_dir,df_summ_3mo,'3month',75,latest,city,state,lat,long,elev)
        dPdt_wind_temp(plots_dir,df_rawl_yer,'1year',75,latest,city,state,lat,long,elev)
        #dPdt_wind_temp(plots_dir,df_summ_all,'all',75,latest,city,state,lat,long,elev)
    except:
        pass
